...
public record ItemAdapter(IntegrationRegistry registry) {
    ...
    public Optional<ForgingItem> toForgery(ItemStack itemStack) {
        ...
        TemperatureData temperatureData = HeatBehavior.fromItem(itemStack)
                .map(heatBehavior -> new TemperatureData(heatBehavior, temperature, timeStamp))
                .orElse(null);
        return Optional.ofNullable(view.get(PdcKeys.FORGING_STEPS, ForgingStepsPersistentData.INSTANCE))
                .map(forgingSteps -> new ForgingItem(materialOptional.orElse(null), forgingSteps, temperatureData))
                .or(() ->
                        materialOptional.map(material ->
                                new ForgingItem(material, new ForgingSteps(List.of()), temperatureData)
                        )
                );
    }
    ...
    public ItemStack toBukkit(ForgingItem item) {
        ...
        if (item.temperatureData() != null) {
            HeatBehavior heatBehavior = item.temperatureData().behavior();
            heatBehavior.applyTo(output, heatBehavior.computeNewTemperature(item.temperatureData().temperature(), item.temperatureData().timestamp()));
        }
        return output;
    }
